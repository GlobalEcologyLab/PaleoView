...
class ApplicationGUI(tk.Frame) :
    ...
    def selectRegion(self, selected) :
        ...
        if selected == 'user-defined' :
            ...
            if self.region_is_time_dependent[selected] :
                self.region_mask = {}
                mask_year = self.calculateTimeDependentRegionYear()
                self.region_mask[mask_year] = self.data_file_helper.loadRegionMask(selected, time_dependent=True, year=mask_year)
            else :
                self.region_mask = self.data_file_helper.loadRegionMask(selected)
    ...
    def createRegionFigure(self, update=False, user_defined_update=False, time_dependent_update=False, time_dependent_year=None) :
        ...
        if self.region_is_time_dependent[self.current_region] and time_dependent_year != None :
            if not self.region_mask.has_key(time_dependent_year) :
                self.region_mask[time_dependent_year] = self.data_file_helper.loadRegionMask(self.current_region, time_dependent=True, year=time_dependent_year)
    ...
    def updateTimeDependentPreviousNext(self, mask_year) :
        previous_mask_year = mask_year + self.calculateTimeDependentRegionYearStep()
        if previous_mask_year <= 20995 :
        ...
        next_mask_year = mask_year - self.calculateTimeDependentRegionYearStep()
        if next_mask_year >= 5 :
    ...
    def calculateTimeDependentRegionYearStep(self) :
        if self.current_valid_interval_step_value > 10 :
            return int(round(self.current_valid_interval_step_value/10.0,0)*10)
        else :
            return 10
    ...
    def viewPlot(self) :
        ...
        # Get all required time dependent masks
        if self.region_is_time_dependent[self.current_region] :
            for i, interval_from in enumerate(range(period_ad_from, period_ad_until+1, self.current_valid_interval_step_value)) :
                time_dependent_year = self.data_file_helper.nearestTimeDependentRegionMaskYear(1950 - interval_from)
                if not self.region_mask.has_key(time_dependent_year) :
                    self.region_mask[time_dependent_year] = self.data_file_helper.loadRegionMask(self.current_region, time_dependent=True, year=time_dependent_year)
    ...
    def createGridPlots(self, parameter_data, region_masks, figure_width=None, figure_height=None) :
        ...
        # Basemap resolution: c (crude), l (low), i (intermediate), h (high), f (full)
        if self.current_region in ['globe', 'land', 'land-0-21KBP', 'ocean', 'ocean-0-21KBP', 'northern-hemisphere', 'southern-hemisphere', 'equatorial-pacific', 'n3', 'n3-4', 'n4', 'rola'] :
            basemap_resolution = 'c'
        elif self.current_region in ['japan', 'middle-east', 'alaska'] :
            basemap_resolution = 'i'
